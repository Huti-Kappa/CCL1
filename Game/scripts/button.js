...
export class Button {
    ...
    draw() {
        this.color = this.backgroundcolor; // Standardfarbe ist die Hintergrundfarbe
        if (global.currentScreen == this.screenID) {
            if (this.click() == true) {
                this.color = "rgba(128, 128, 128, 0.5)"; // Beispiel: Halbtransparentes Grau, wenn geklickt
            }
    
            // Setze die Farbe für den Button-Hintergrund
            global.ctx.fillStyle = this.color;
            global.ctx.fillRect(this.x, this.y, this.width, this.height);
    
            // Setze die Schriftfarbe
